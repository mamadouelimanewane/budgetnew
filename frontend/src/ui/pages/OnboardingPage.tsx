...
const MODS = [
  {id:1,title:"Prise en main",desc:"Interface et navigation.",dur:"15 min",level:"Debutant",prog:100,color:"#10B981",bg:"#D1FAE5"},
  {id:2,title:"Budget UEMOA",desc:"Nomenclature et allocations.",dur:"30 min",level:"Intermediaire",prog:80,color:"#1A6FD4",bg:"#EBF4FF"},
  {id:3,title:"Marches DCMP",desc:"Procedures et appels offres.",dur:"45 min",level:"Intermediaire",prog:40,color:"#7C3AED",bg:"#EDE9FE"},
  {id:4,title:"IA et Previsions",desc:"Forecasting et anomalies.",dur:"20 min",level:"Avance",prog:0,color:"#0E9E8A",bg:"#E6F8F6"},
  {id:5,title:"Reporting TOFE",desc:"Rapports reglementaires BCEAO.",dur:"25 min",level:"Avance",prog:0,color:"#F59E0B",bg:"#FEF3C7"},
];
const CERTIFS = [
  {title:"Gestionnaire certifie",modules:"1 et 2",obtained:true,date:"2026-04-15"},
  {title:"Expert DCMP",modules:"1, 2 et 3",obtained:false},
  {title:"Analyste IA",modules:"1, 2, 4 et 5",obtained:false},
];
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><h1 style={{fontSize:22,fontWeight:700,color:"var(--bn-text)",margin:0}}>Formation et Certification</h1>
        <p style={{fontSize:13,color:"var(--bn-muted)",marginTop:4}}>Modules certifiants en francais et wolof</p></div>
        <div style={{background:"linear-gradient(135deg,#0D2B4B,#1A3352)",borderRadius:10,padding:"8px 18px",textAlign:"center"}}>
          <p style={{fontSize:10,color:"rgba(255,255,255,0.6)"}}>Progression globale</p>
      ...
      <div style={{background:"white",borderRadius:14,padding:"14px 18px",border:"1px solid var(--bn-border)"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
          ...
          <span style={{fontSize:11,color:"var(--bn-muted)"}}>{MODS.filter(m=>m.prog===100).length}/{MODS.length} modules completes</span>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        {MODS.map(m=>(
          <div key={m.id} onClick={()=>setActive(active===m.id?null:m.id)} style={{background:"white",borderRadius:14,padding:18,border:"1px solid",borderColor:active===m.id?m.color:"var(--bn-border)",cursor:"pointer"}}>
            ...
            {active===m.id&&<button style={{marginTop:10,width:"100%",padding:"7px",borderRadius:7,background:m.color,color:"white",border:"none",cursor:"pointer",fontSize:12,fontWeight:600}}>
      ...
      <div style={{background:"white",borderRadius:14,padding:20,border:"1px solid var(--bn-border)"}}>
        <p style={{fontSize:14,fontWeight:600,marginBottom:14}}>Certifications disponibles</p>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
          {CERTIFS.map((c,i)=>(
            <div key={i} style={{borderRadius:12,padding:14,border:"1px solid",borderColor:c.obtained?"#10B981":"var(--bn-border)",background:c.obtained?"linear-gradient(135deg,#065F46,#0F6E56)":"white"}}>
              <p style={{fontSize:24,textAlign:"center",marginBottom:8}}>{c.obtained?"medal":"lock"}</p>
              <p style={{fontSize:12,fontWeight:600,color:c.obtained?"white":"var(--bn-text)",textAlign:"center"}}>{c.title}</p>
              <p style={{fontSize:10,color:c.obtained?"rgba(255,255,255,0.6)":"var(--bn-muted)",textAlign:"center",marginTop:4}}>Modules : {c.modules}</p>
            </div>
          ))}
        </div>
      </div>
