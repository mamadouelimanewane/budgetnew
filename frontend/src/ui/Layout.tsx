...
const NAV: { section: string; color?: string; items: NavItem[] }[] = [
  ...
  { section: "Elite", color: "#EF4444", items: [
    { to: "/copilote", label: "Copilote IA Claude", tag: "NEW" },
    { to: "/carte", label: "Geo-Dashboard", tag: "NEW" },
        ...
        <nav style={{ padding: "8px 8px 0", flex: 1 }}>
          {NAV.map(section => (
            <div key={section.section}>
              <p style={{ fontSize: 9, fontWeight: 600, color: section.color || "#475569", padding: "8px 10px 5px", textTransform: "uppercase", letterSpacing: "0.08em" }}>{section.section}</p>
              {section.items.map(item => (
                <NavLink key={item.to} to={item.to} style={({ isActive }) => ({
                  ...
                  background: isActive ? (section.color ? section.color + "33" : "rgba(26,111,212,0.25)") : "transparent",
                  color: isActive ? (section.color === "#EF4444" ? "#FCA5A5" : section.color === "#7C3AED" ? "#C4B5FD" : "#7BC8FF") : "#94A3B8",
