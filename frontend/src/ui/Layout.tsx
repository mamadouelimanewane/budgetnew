...
const NAV: { section: string; color?: string; items: NavItem[] }[] = [
  { section:"Standard", items:[
    ...
    {to:"/procurement",  label:"Achats & Paiements"},
    {to:"/revenue",      label:"Recettes"},
    {to:"/exports",      label:"Exports & Rapports"},
  ...
  { section:"Premium âœ¦", color:"#7C3AED", items:[
    ...
    {to:"/license",      label:"Plans & Licence"},
  ]},
];
        ...
        <nav style={{padding:"8px 8px 0",flex:1}}>
          {NAV.map(section=>(
            <div key={section.section}>
              <p style={{fontSize:9,fontWeight:600,color:section.color||"#475569",padding:"10px 10px 5px",textTransform:"uppercase",letterSpacing:"0.08em"}}>{section.section}</p>
              {section.items.map(item=>(
                <NavLink key={item.to} to={item.to} style={({isActive})=>({
                  display:"flex",alignItems:"center",justifyContent:"space-between",padding:"7px 10px",borderRadius:6,marginBottom:1,
